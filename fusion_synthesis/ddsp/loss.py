...
class PerceptualLoss(nn.Module):
    def __init__(self, n_ffts: int, jitter_weight: float = 10, shimmer_weight: float = 0.1, use_kurtosis: bool = False,
                 use_emo_loss: bool = False):
        super().__init__()
...
class MSSLoss(nn.Module):
    ...
    Usage ::
    mssloss = MSSLoss([1024, 512, 256, 128, 64, 32], alpha=1.0, overlap=0.75)
    mssloss(y_pred, y_gt)
    ...
    def __init__(self, n_ffts, alpha=1.0, ratio=1.0, overlap=0.75, eps=1e-7, use_kurtosis=True):
        ...
        self.losses = nn.ModuleList(
            [SSSLoss(n_fft, alpha, overlap, eps, use_kurtosis=use_kurtosis) for n_fft in n_ffts])
        self.ratio = ratio
...
class JitterLoss(nn.Module):
    ...
    # def _get_local_jitters(self, f0_hz_true, f0_predict):
    ...
    #     jitter_true = jitter_true_abs / (f0_true_period.mean(1))
    #
    ...
    #     jitter_pred = jitter_pred_abs / (f0_pred_period.mean(1))
    #
    #     return jitter_true, jitter_pred

    def _get_five_point_jitters(self, f0_hz_true, f0_predict):
        f0_true_period = torch.where(f0_hz_true == 0., self.eps, 1 / (f0_hz_true + self.eps))
        jitter_true = _get_five_point_jitter(f0_true_period)

        f0_pred_period = torch.where(f0_predict == 0., self.eps, 1 / (f0_predict + self.eps))
        jitter_pred = _get_five_point_jitter(f0_pred_period)
...
class MAELoss(nn.Module):
    """
    MAE loss used for emotion loss
    """
...
class ShimmerLoss(nn.Module):
    ...
    def forward(self, x_pred, x_true, is_val, max_iteration):
        ...
        if self.iteration > max_iteration:
            shimmer_pred, shimmer_true = _get_shimmer(x_pred, x_true)
...
def _peak_detect(x, window_size=100):
...
def _get_shimmer(x_true, x_pred, eps=1e-7):
    max_amplitude_t, min_amplitude_t, mean_amplitude_t = _peak_detect(x_true)
    max_amplitude_p, min_amplitude_p, mean_amplitude_p = _peak_detect(x_pred)
    # relative shimmer
