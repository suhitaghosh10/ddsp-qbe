...
from utils import BLACKMAN, BLACKMAN_HARRIS, HANN, CUDA
...
def unit_to_F0(unit,
               ...
               f0_max,
               use_log=True):
    """Map unit interval [0, 1] to [hz_min, hz_max], optional scaling logarithmically."""
    if use_log:
...
def fft_convolve(audio,
    ...
    Args:
        :param audio: Input audio. Tensor of shape [batch, audio_timesteps].
        :param impulse_response: Finite impulse response to convolve. Can either be a 2-D
        Tensor of shape [batch, ir_size], or a 3-D Tensor of shape [batch,
        ir_frames, ir_size]. A 2-D tensor will apply a single linear
        time-invariant filter to the audio. A 3-D Tensor will apply a linear
        time-varying filter. Automatically chops the audio into equally shaped
        blocks to match ir_frames.
        :param padding: Either 'valid' or SAME. For SAME the final output to be the
        same size as the input audio (audio_timesteps). For 'valid' the audio is
        extended to include the tail of the impulse response (audio_timesteps +
        ir_timesteps - 1).
        :param delay_compensation: Samples to crop from start of output audio to compensate
    ...
    if frame_size != audio_size:
        ...
        output_signal = crop_and_compensate_delay(audio_frames_out.squeeze(1), audio_size, ir_size, padding,
                                                  delay_compensation)
    return output_signal[..., :frame_size * n_ir_frames]  # Crop and shift the output audio.
...
def linear_lookup(index,
    ...
    """
    Args:
        :param index: B x T, value: [0, 1]
        :param wavetable: B x num_wavetables x len_wavetable
    :return:
...
def crop_and_compensate_delay(audio: torch.Tensor, audio_size: int, ir_size: int,
  ...
  Args:
    :param audio: Audio after convolution. Tensor of shape [batch, time_steps].
    audio_size: Initial size of the audio before convolution.
    :param ir_size: Size of the convolving impulse response.
...
def apply_window_to_impulse_response(impulse_response,
    ...
    Args:
        :param impulse_response: A series of impulse responses frames to window, of shape
        [batch, n_frames, ir_size]. ---------> ir_size means size of filter_bank ??????

        :param window_size: Size of the window to apply in the time domain. If window_size
        is less than 1, it defaults to the impulse_response size.
        :param causal: Impulse response input is in causal form (peak in the middle).
        :param window_type: allowed options: blackman-harris, blackman, hann
        :param convolve_power: how many times the window self-multiplied.
        :param is_odd: odd or even window
        :param device: cuda or cpu
    ...
    window = nn.Parameter(
        get_window(wtype=window_type, convolve_power=convolve_power, window_width=window_size, device=device),
        requires_grad=False)
    if device == CUDA:
        window = window.cuda()
...
def frequency_impulse_response(magnitudes,
    ...
    Args:
        :param magnitudes: Frequency transfer curve. Float32 Tensor of shape [batch,
...
def blackman_harris_window(n, dtype=torch.float32, device=CUDA):
    """
    The torch version of blackman-harris window.
    :param n: the width of window
    :param dtype: data type
    :param device: cuda or cpu
    :return:
        Window of the specified n shape
    """
    if n <= 1:
    ...
    window = a0 - a1 * torch.cos(2 * math.pi * t / (n - 1)) + a2 * torch.cos(
        4 * math.pi * t / (n - 1)) - a3 * torch.cos(6 * math.pi * t / (n - 1))

    return window


def get_window(wtype, convolve_power, window_width, device=CUDA):
    """

    :param wtype: window type
    :param convolve_power: how many times the window self-multiplied.
    :param window_width: the width of window
    :param device: cuda or cpu
    :return:
    """
    if convolve_power < 1:
        raise Exception('please provide the power >=1 for blackman-harris window')
    else:
        if wtype == HANN:
